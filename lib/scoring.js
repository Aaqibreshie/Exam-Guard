...
export async function scoreAnswerAsync(question, studentAnswer) {
  ...
  if (question.question_type === 'coding' || question.question_type === 'project') {
    ...
    if (testCases.length === 0 && question.options && typeof question.options === 'object' && Array.isArray(question.options.test_cases)) {
      testCases = question.options.test_cases;
    }
    if (testCases.length > 0) {
      try {
        const testReport = await runTestCases(studentAnswer, testCases);
